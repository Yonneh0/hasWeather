...
- localStorage-based caching with configurable TTLs per data type (15 min for weather/AQI, 24h for geocoding/IP, 10 min for nearby cities)
...
- Donkey Runner minigame — a Chrome dinosaur-style endless runner with a hand-drawn donkey character, obstacles, scoring, combo/multiplier system, near-miss detection, day/night cycle, particle effects, and procedural sound effects
...
index.html              71 lines — development HTML template (linked CSS/JS)
weather.html            5691 lines — release build (everything bundled together; auto-generated by build.js)
...
css/
  ...
  card-header.css       68 lines — card header (city name, temp, weather icon)
  card-details.css      86 lines — info rows, details grid, AQI badge, sun times
  hourly.css            64 lines — hourly forecast grid & slots
  daily.css             74 lines — 7-day forecast rows & temperature bars
  ...
  donkey-runner.css     449 lines — minigame panel styling & animations
...
js/
  ...
  cache.js              93 lines — DataCache (localStorage caching with configurable TTLs)
  ...
  render.js             252 lines — DOM rendering (city cards, hourly/daily forecasts)
  ...
  favorites-ui.js       440 lines — favorites dropdown UI & city management
  network-monitor.js    331 lines — network outage detection, animated error panel, auto-retry
  donkey-runner.js      1442 lines — Donkey Runner minigame engine (canvas-based runner)
rag-docs/
...
Clear all cached data via browser dev tools → Application → Local Storage → remove keys starting with `hasw_cache_` or `hasW_`.
